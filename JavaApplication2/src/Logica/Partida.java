...
public class Partida {
    ...
    public void ponerBarcos(Tablero tabPlayer1, Tablero tabPlayer2) {
        ...
        for (int i = 2; i <= 6; i++) {
            for (int j = 1; j <= 2; j++) {
                ...
                dir = rnd.nextBoolean();
                barcosPlayer2[cont].setPos(tabPlayer2.ubicarBarco(barcosPlayer2[cont]));
                cont++;
    ...
    public Celda[][] getTableroA(){
        return tabPlayer1.getTablero();
    }
    
    public Celda[][] getTableroB(){
        return tabPlayer2.getTablero();
    }
    ...
    public double[] porcentajesAciertos() {
        ...
        double aux;
        aux=(impactos1*1.0) / (disparos1*1.0);
        //aux=(Math.round(aux*100))/100;
        porcentajes[0] = ((int)aux*10000)/100d;
        aux=(impactos2*1.0) / (disparos1*1.0);
        //aux=(Math.round(aux*100))/100;
        porcentajes[1]=((int)aux*10000)/100d;
        aux=(errados1*1.0) / (disparos1*1.0);
        //aux=(Math.round(aux*100))/100;
        porcentajes[2] =((int)aux*10000)/100d;
        aux=(errados2*1.0) / (disparos1*1.0);
        //aux=(Math.round(aux*100))/100;
        porcentajes[3] =((int)aux*10000)/100d;
        return porcentajes;
    }
