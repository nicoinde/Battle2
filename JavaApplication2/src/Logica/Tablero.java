...
public class Tablero {
        ...
        if (horizontal) {
            if (y + tamaño < TAMAÑO_TABLERO) {
                for (int i = y; i < y + tamaño; i++) {
                    int aux1 = tablero[x][i].getActual();
                    if (aux1 == 1) {
                        return false;
                    }
                }
            ...
            if (x + tamaño < TAMAÑO_TABLERO) {
                for (int i = x; i < x + tamaño; i++) {
                    int aux1 = tablero[i][y].getActual();
                    if (aux1 == 1) {
                        return false;
                    }
                }
    ...
    public boolean impacto(int x, int y){
        ...
        if(tablero[x][y].getActual()==0){
            tablero[x][y].setActual(3);
            return false;
        }
        tablero[x][y].setActual(2);
        return true;
    }
...
//</editor-fold>
    public int[] ubicarBarco(Barco bote) {
        ...
        if (bote.isHorizontal()) {
            if (y + tamaño < TAMAÑO_TABLERO) {
                for (int i = y; i < y + tamaño; i++) {
                    tablero[x][i].setActual(1);
                }
            }
        } else {
            if (x + tamaño < TAMAÑO_TABLERO) {
                for (int i = x; i < x + tamaño; i++) {
                    tablero[i][y].setActual(1);
